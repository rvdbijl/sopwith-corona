...
static bool LoadHighScores(const char *filename)
{
	...
	while (idx < MAX_HIGH_SCORES && ReadLine(fs, line, sizeof(line))) {
		...
		if (sscanf(line, "%3s %d %d %d", hs->name, &hs->score.score,
		           &medals, &ribbons) < 4) {
			continue;
		}
		hs->score.medals_nr = UnpackMedals(medals, hs->score.medals, 3);
		hs->score.ribbons_nr =
...
static bool SaveHighScores(const char *filename)
{
	...
	fprintf(fs, "# Sopwith high scores file, generated by " PACKAGE_STRING
	            "\n");
...
static const char *HighScoreFilePath(void)
{
	...
	if (strlen(HISCORES_PATH) > 0) {
		...
		if (errno != ENOENT) {
			fprintf(stderr,
			        "Error opening the system-wide high "
...
static void DrawHighScore(const struct high_score *hs, int x, int y)
{
	...
	swposcur(x, y);
	snprintf(buf, sizeof(buf), "%-3.3s ....... %-4d", hs->name,
	         hs->score.score);
	swputs(buf);
	...
	for (m = 0; m < hs->score.medals_nr; ++m) {
		...
		Vid_DispSymbol(px + m * 8, py, &ss->sym[0], FACTION_PLAYER1);
	}
...
void DrawHighScoreTable(void)
{
	...
	Vid_Box(TABLE_X * 8 - 8, SCR_HGHT - TABLE_Y * 8 + 4, TABLE_W * 8 + 8,
	        TABLE_H * 8, 0);
...
bool NewHighScore(score_t *s)
{
	...
	if (playmode != PLAYMODE_COMPUTER || currgame != &original_level ||
